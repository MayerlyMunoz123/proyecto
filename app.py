...
def update():
    ...
    cursor = conn.cursor(cursor_factory=extras.RealDictCursor)
    cursor.execute("SELECT idambiente, descripcion, nombreambiente FROM ambientes")
    suerte = cursor.fetchall()
    conn.close()
    cursor.close()
    print(suerte)
    return render_template("update.html", suerte=suerte)
...
def eliminar_reserva(idreserva):
    ...
    if request.method == "POST":
        if eliminar:
            ...
            conn.close()
            return redirect("/registro")
    return render_template("delete.html",reservas=eliminar)

#Ruta para editar un registro
@app.route("/<int:idreserva>/edit", methods=["GET", "POST"])
def editar_registro(idreserva):
    conn=connect_database()
    cursor=conn.cursor(cursor_factory=extras.RealDictCursor)
    cursor.execute("Select * from reservas where idreserva=%s", (idreserva,))
    ...
    if request.method=="POST":
        ...
        if editado:
            ...
            conn.close()
            return redirect("/registro")
    return render_template("update.html", reserva=editar)
